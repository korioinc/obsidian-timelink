...
void test('buildTimelineResizeCommitEntries builds previous/next pair from resize hover', () => {
	...
	assert.equal(entries.previous[0].endTime, '10:00');
	assert.equal(entries.next[0].endTime, '11:00');
	assert.equal(entries.next[1], entries.previous[1]);
});
...
void test('hasTimelineResizeChange returns false when resize does not change final event', () => {
	...
	assert.equal(hasTimelineResizeChange(entries.next, entries.previous), false);
});
...
void test('hasTimelineResizeChange returns true when resize changes end date or time', () => {
	...
	assert.equal(hasTimelineResizeChange(entries.next, entries.previous), true);
});
