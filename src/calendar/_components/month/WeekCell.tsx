import { formatDateKey, isToday } from '../../../shared/event/model-utils';
import type { AllDayEventInteractionHandlers } from '../../all-day-interaction-types';
import type { EventSegment } from '../../types';
import { getMonthAllDayMarginClass } from '../../utils/all-day-event-bar';
import type { DayCellData } from '../../utils/date-grid';
import { EVENT_ROW_GAP, EVENT_ROW_HEIGHT } from '../../utils/month-calendar-utils';
import { getWeekEventLayout } from '../../utils/week-event-layout';
import { AllDayEventBar } from '../day-week/AllDayEventBar';
import { useEffect, useMemo, useRef, useState } from 'preact/hooks';

type WeekCellData = DayCellData & { key: string };

type WeekCellProps = AllDayEventInteractionHandlers & {
...
export const WeekCell = ({
	...
	const renderAllDayBars = (
		placements: typeof visibleMultiDayPlacements,
		keyPrefix: 'multi' | 'single',
	) =>
		placements.map((placement) => (
			<AllDayEventBar
				key={`${keyPrefix}-${placement.segment.id}-${placement.weekRow}-${placement.columnStart}`}
				placement={placement}
				...
				marginClassName={getMonthAllDayMarginClass(placement)}
	...
	return (
		<div className="relative grid h-full min-h-0 grid-cols-7 gap-px" ref={containerRef}>
			...
			<div className="pointer-events-none absolute inset-x-0 top-10 bottom-0 z-20" ref={eventsRef}>
				...
				>
					{renderAllDayBars(visibleMultiDayPlacements, 'multi')}
					{renderAllDayBars(visibleSingleDayPlacements, 'single')}
					{hasAnyMore &&
